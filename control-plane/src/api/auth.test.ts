...
import { getMe, beginEnroll, finishEnroll, login, EnrollError } from "./auth";
...
describe("auth api", () => {
  it("getMe returns the human on 200, null on 401", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => res(200, { kind: "human", name: "edwin" })));
  ...
  it("beginEnroll throws bad-code on 410", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => res(410, { error: "invalid-code" })));
  ...
  it("login runs begin → ceremony → verify", async () => {
    const fetchMock = vi.fn(async (url: RequestInfo | URL) =>
      String(url).endsWith("/auth/login/verify") ? res(200, { userId: "u1", name: "edwin" }) : res(200, { challenge: "c" }));
    vi.stubGlobal("fetch", fetchMock);
