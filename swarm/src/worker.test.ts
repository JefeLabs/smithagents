...
test("mergeWorkerConfig: explicit undefined overrides never clobber defaults", () => {
  ...
  const merged = mergeWorkerConfig({
    orchestratorUrl: "ws://x:1",
    token: "t",
    capacity: undefined,
    name: undefined,
...
test("nextReconnectDelay: exponential from base, capped, jittered ±20%", () => {
  ...
  assert.equal(
    nextReconnectDelay(2, 3_000, 60_000, () => 0),
    9_600,
  );
  assert.equal(
    nextReconnectDelay(2, 3_000, 60_000, () => 1),
    14_400,
  );
});
...
test("registerDevice surfaces a rejected code as an error", async () => {
  ...
  const port = (server.address() as { port: number }).port;
  const credsPath = join(await mkdtemp(join(tmpdir(), "creds-")), "worker-credentials.json");
  await assert.rejects(registerDevice(`http://127.0.0.1:${port}`, "DEAD-CODE", "box", credsPath), /Invalid or expired/);
  server.close();
