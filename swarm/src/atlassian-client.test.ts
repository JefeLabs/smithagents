...
test("lookupTicket: ok returns key/summary/status/url", async () => {
  const f = (async (url: unknown) => {
    ...
    return new Response(
      JSON.stringify({ key: "PROJ-123", fields: { summary: "Fix the thing", status: { name: "In Progress" } } }),
      {
        status: 200,
      },
    );
...
test("lookupTicket: not found surfaces a readable detail", async () => {
  const f = (async () =>
    new Response(JSON.stringify({ errorMessages: ["Issue does not exist"] }), { status: 404 })) as typeof fetch;
...
test("searchDocs: scopes CQL to configured space keys and returns title/url pairs", async () => {
  ...
  assert.deepEqual(r.docs, [
    { title: "Onboarding", excerpt: "", url: "https://acme.atlassian.net/wiki/spaces/DOCS/pages/1/Onboarding" },
  ]);
});
