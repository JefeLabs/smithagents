...
async function cmdSquadSubmit(args: string[]): Promise<void> {
  ...
  if (!opts.prompt) {
    console.error(
      'Usage: smith squad submit --prompt "..." [--squad alpha|beta|gamma] [--mode solo|squad|council] [--agents 2-4]',
    );
    process.exit(1);
  }
...
async function cmdSquadList(): Promise<void> {
  ...
  for (const s of d.squads) {
    ...
    console.log(
      `  ${color}● ${s.id.padEnd(8)}\x1b[0m ${color}(${leader}→)\x1b[0m ${s.taskId ? `[Task: ${s.taskId}]` : ""}`,
    );
  }
  console.log();
}
