...
class Dashboard {
  ...
  private render(): void {
    ...
    // ── Header ──
    const wsStatus = this.state.connected ? ansi.green("● WS") : ansi.red("○ WS");
    const title = ` ${ansi.bold("SMITH ORCHESTRATOR")} `;
    const healthTag = this.state.health ? ansi.bgGreen(" ONLINE ") : ansi.bgRed(" OFFLINE ");
    ...
    for (let i = 0; i < ROSTER.length; i++) {
      ...
      if (seat && seat.taskId) {
        ...
        const locStr =
          loc === "remote" ? ansi.magenta(pad(loc, 8)) : loc === "docker" ? ansi.cyan(pad(loc, 8)) : pad(loc, 8);
        ...
        const highlight = i === this.state.selectedIdx ? (s: string) => ansi.cyan(s) : (s: string) => ansi.dim(s);
        lines.push(`${sel} ${idx}  ${highlight(pad(name, 12))} ${ansi.dim(pad("-", 8))} ${ansi.dim("○ idle")}`);
      }
    }
    ...
    if (completed.length > 0) {
      ...
      for (const t of completed.slice(0, 5)) {
        const icon = t.status === "completed" || t.exitCode === 0 ? ansi.green("✓") : ansi.red("✗");
...
const isDirect = process.argv[1]?.endsWith("dashboard.ts") || process.argv[1]?.endsWith("dashboard.js");
if (isDirect) {
  main().catch((err) => {
    process.stdout.write(ansi.cursorShow);
    console.error(err);
