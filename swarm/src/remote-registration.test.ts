...
function reg(overrides: Partial<WorkerRegisterMessage>): WorkerRegisterMessage {
  return {
    type: 'register', workerId: 'self-asserted', name: 'box', secret: '',
    capacity: 5, agents: ['claude'], runtimes: ['tmux'], version: '0.1.0',
    ...overrides,
  };
}
...
test('a bad token never falls through to the secret path', async () => {
  ...
  const verdict = await evaluateWorkerRegistration(
    reg({ token: 'smith-device-bogus', secret: 'legacy-secret' }), registry, configured);
