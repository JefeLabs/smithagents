...
const FIXTURE = [
  ...
  JSON.stringify({
    ...
    message: {
      role: "assistant",
      content: [{ type: "text", text: "Fixed — the widget renders." }],
...
test("the agent definition determines the launched process: model flag on both paths", () => {
  ...
  assert.match(
    driver.taskCommand("claude --dsp", "do it", "claude-opus"),
    /--model claude-opus .*--print 'do it'|--model claude-opus/,
  );
});
...
test("materialize: never overwrites a .mcp.json the repo already tracks", async () => {
  ...
  try {
    ...
    await git("git", ["-c", "user.name=t", "-c", "user.email=t@t", "commit", "-q", "-m", "own mcp config"], {
      cwd: dir,
    });
