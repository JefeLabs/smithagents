...
export async function verifyElevenlabs(apiKey: string, fetchImpl: typeof fetch = fetch): Promise<VerifyResult> {
  try {
    ...
    const body = (await res.json().catch(() => ({}))) as { voices?: unknown[]; detail?: { status?: string; message?: string } };
    ...
    if (res.status === 401 && body.detail?.status === 'missing_permissions') {
      return { ok: true, detail: 'Key valid — restricted scope: voice list blocked, grant "Voices: Read" to enable voice casting' };
    }
