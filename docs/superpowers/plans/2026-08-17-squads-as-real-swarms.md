...
> **Resolved in this branch (2026-08-17, ruled by Edwin).** Task 5 found this to be worse than predicted. The stale registrations were real but cosmetic — they are `prunable`, and `addMemberWorktrees` already prunes before adding, so a later add self-heals. The **data loss** was not cosmetic: `instanceIsDirty` never inspected `members/*`, so `destroyInstance` **without** `force` deleted a member's uncommitted work without the refusal every other member gets — violating this plan's own Global Constraint "Never destroy uncommitted work". Proven in a temp repo: destroy neither refused nor preserved the file. Fixed with full parity — member worktrees now join the dirty check (qualified as `members/<name>` in the refusal) and are deregistered by pruning each source after the instance directory is removed. Two regression tests cover the refusal and the absence of stale registrations.
