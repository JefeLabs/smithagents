...
import { classifyBump, githubAtomUrl, latestVersion, mentionsSecurity, qualifies, repositoryUrl } from "./versions.ts";
...
test("githubAtomUrl reaches the free Atom feed rather than the rate-limited API", () => {
  ...
  assert.equal(
    githubAtomUrl("https://github.com/tauri-apps/tauri"),
...
test("repositoryUrl reads npm and cargo; maven has none, so maven releases carry no notes", async () => {
  assert.equal(
    await repositoryUrl(
      async () => ({ repository: { url: "git+https://github.com/facebook/react.git" } }),
      "npm",
      "react",
    ),
  ...
  assert.equal(
    await repositoryUrl(
      async () => ({ crate: { repository: "https://github.com/tauri-apps/tauri" } }),
      "cargo",
      "tauri",
    ),
