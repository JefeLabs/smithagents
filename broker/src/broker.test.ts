...
import { type BridgeLike, Broker, type SttLike, type SwarmClientLike, TTS_SAMPLE_RATE } from "./broker.ts";
    ...
    listSquads: async () => [
      {
        id: "alpha",
        status: "idle",
        leader: { name: "Gabriel", role: "leader" },
        members: [{ name: "Gabriel", role: "leader" }],
      },
      {
        id: "beta",
        status: "idle",
        leader: { name: "Gustavo", role: "leader" },
        members: [{ name: "Gustavo", role: "leader" }],
      },
    ],
    ...
    listWorkspaces: async () => [
      {
        name: "jefelabs",
        default: true,
        repos: [{ name: "smithagents", path: "/path/to/smithagents", branch: "main" }],
      },
    ],
...
test("delegate executor keeps its exact success/refusal strings", async () => {
  ...
  assert.equal(
    ok,
    "Delegated to Manuel: task t-77 queued. They will work asynchronously; you will be notified on completion.",
  );
...
test("roster composition persists: a second broker restores groups and edits, dropping stale ids", async () => {
  ...
  const store = {
    load: () => saved,
    save: (s: import("./broker.ts").RosterState) => {
      saved = s;
    },
  };
  const f1 = makeFakes([]);
...
test("search_docs lists title/url pairs, or a no-results line", async () => {
  const f = makeFakes([]);
  f.swarm.searchDocs = async () => ({
    ok: true,
    docs: [{ title: "Onboarding", excerpt: "", url: "https://x/wiki/1" }],
  });
  const b = makeBroker(f);
  assert.equal(
    await b.executors.search_docs({ query: "onboarding", workspace: "acme" }),
    "Onboarding — https://x/wiki/1",
  );
  ...
  const b2 = makeBroker(f2);
  assert.equal(
    await b2.executors.search_docs({ query: "nothing", workspace: "acme" }),
    'No Confluence docs found for "nothing".',
  );
});
