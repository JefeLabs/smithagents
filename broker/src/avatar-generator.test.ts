...
const tinyPng = () =>
  sharp({ create: { width: 4, height: 4, channels: 3, background: "#aa5533" } })
    .png()
    .toBuffer();
...
test("buildAvatarPrompt: house style always present, persona clauses appended", () => {
  const p = buildAvatarPrompt({
    ...
    backstory: "Found her first injection hole at nineteen.",
  });
...
test("generate: normalizes whatever Gemini returns to a 512x512 png (base64)", async () => {
  ...
  const gen = new AvatarGenerator(
    clientReturning([{ inlineData: { data: src, mimeType: "image/png" } }]),
