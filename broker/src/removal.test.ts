...
import { createRemovalService, resolveRemoval, transcriptMentions } from "./removal.ts";
...
test("removal service: aggregates evidence, calls the matching swarm op", async () => {
  ...
  assert.deepEqual(await make([], { warmSessions: 0, activeTasks: 0 }).preview("nobody"), {
    error: "Unknown agent: nobody",
  });
});
