...
test('remember persists; the same key in the same scope updates instead of duplicating', () => {
  ...
  memory.remember({ key: 'pr-style', text: 'Edwin prefers draft PRs with a task summary', scope: { workspace: 'jefelabs' } });
...
test('scope isolation: a session note never leaks into another session', () => {
  const { memory } = makeMemory();
  memory.remember({ key: 'secret-plan', text: 'we are rewriting the composer', scope: { workspace: 'w', session: 's1' } });
...
test('workspace facts reach every session inside that workspace, but not other workspaces', () => {
  ...
  assert.equal(memory.recall({ text: 'which branch do we build off', scope: { workspace: 'jefelabs', session: 'any' } }).length, 1);
  assert.equal(memory.recall({ text: 'which branch do we build off', scope: { workspace: 'other', session: 'any' } }).length, 0);
});
...
test('morphology does not defeat recall: "branches" answers "branch"', () => {
  const { memory } = makeMemory();
  memory.remember({ key: 'branching-source', text: 'Always cut branches from main, never from develop.', scope: { workspace: 'w' } });
  const hits = memory.recall({ text: 'which branch should a new feature branch come off of?', scope: { workspace: 'w' } });
